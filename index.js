  ...
  function (req, res) {
    ...
    request(options, function (error, response, body) {
      ...
      parseString(body, function (err, result) {
        ...
        req.body.quantity > maxServers ? req.body.quantity = maxServers : null

        for (let i = 1; i < req.body.quantity; i++) {
          dns.lookup(req.body.country + '-s' + i + '.netnut.io', function (err, result) {
            if (req.body.type == 'footsites') {
              proxies.push({ username: req.user.customer_login_name + `!a${req.body.country.toLowerCase() == 'us' ? subnets.us.footsites[Math.floor(Math.random() * subnets.us.footsites.length)] : subnets.footsites[Math.floor(Math.random() * subnets.footsites.length)]}` , password: req.user.customer_login_pwd, ip: result, port: 33128 })
            }
            else if (req.body.type == 'nike') {
              proxies.push({ username: req.user.customer_login_name + `!a${req.body.country.toLowerCase() == 'us' ? subnets.us.nike[Math.floor(Math.random() * subnets.us.nike.length)] : subnets.nike[Math.floor(Math.random() * subnets.nike.length)]}` , password: req.user.customer_login_pwd, ip: result, port: 33128 })
            }
            else if (req.body.type == 'adidas') {
              proxies.push({ username: req.user.customer_login_name + `!a${req.body.country.toLowerCase() == 'us' ? subnets.us.adidas[Math.floor(Math.random() * subnets.us.adidas.length)] : subnets.adidas[Math.floor(Math.random() * subnets.adidas.length)]}` , password: req.user.customer_login_pwd, ip: result, port: 33128 })
            }
            else if (req.body.type == 'supreme') {
              proxies.push({ username: req.user.customer_login_name + `!a${req.body.country.toLowerCase() == 'us' ? subnets.us.supreme[Math.floor(Math.random() * subnets.us.supreme.length)] : subnets.supreme[Math.floor(Math.random() * subnets.supreme.length)]}` , password: req.user.customer_login_pwd, ip: result, port: 33128 })
            }
            else if (req.body.type == 'mesh') {
              proxies.push({ username: req.user.customer_login_name + `!a${req.body.country.toLowerCase() == 'us' ? subnets.us.mesh[Math.floor(Math.random() * subnets.us.mesh.length)] : subnets.mesh[Math.floor(Math.random() * subnets.mesh.length)]}` , password: req.user.customer_login_pwd, ip: result, port: 33128 })
            }
            else if (req.body.type == 'sticky') {
              proxies.push({ username: req.user.customer_login_name + `!a${Math.floor(Math.random() * 256) + 1}` , password: req.user.customer_login_pwd, ip: result, port: 33128 })
            }
            else {
              proxies.push({ username: req.user.customer_login_name, password: req.user.customer_login_pwd, ip: result, port: 33128 })
            }
            genned++
            if (genned == req.body.quantity - 1) {
              res.send(proxies)
            }
          })
        }
